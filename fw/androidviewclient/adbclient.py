...
class WifiManager:
    ...
    def getWifiState(self):
        ...
        @return: One of WIFI_STATE_DISABLED, WIFI_STATE_DISABLING, WIFI_STATE_ENABLED, WIFI_STATE_ENABLING,
        WIFI_STATE_UNKNOWN
        """
...
class AdbClient:
    ...
    def setAlarm(timeout):
        ...
        # Shawn Wang: consider disable it due to unpredictable time required to dump complex views
    ...
    def checkVersion(self, ignoreversioncheck=False, reconnect=True):
        ...
        version = AdbClient.__readExactly(self.socket, 8)
    ...
    def getDevices(self):
        ...
        try:
            ...
            return None
        devices = []
        for device_line in self.__receive().splitlines():
            devices.append(Device.factory(device_line))
        self.__connect()
        return devices

    def shell(self, shell_cmd=None):
        if DEBUG:
            print >> sys.stderr, "shell(cmd=%s)" % shell_cmd
        self.__checkTransport()
        if shell_cmd:
            self.__send('shell:%s' % shell_cmd, checkok=True, reconnect=False)
            out = ''
            while True:
                _str = None
                try:
    ...
    def __getRestrictedScreen(self):
        ...
        for dumped_line in self.shell('dumpsys window').splitlines():
            m = rsRE.match(dumped_line)
            if m:
                return m.groups()
    ...
    def getLogicalDisplayInfo(self):
        ...
        for dumped_line in self.shell('dumpsys display').splitlines():
            m = logicalDisplayRE.search(dumped_line, 0)
            if m:
                self.__displayInfo = {}
    ...
    def getPhysicalDisplayInfo(self):
        ...
        for dumped_line in self.shell('dumpsys display').splitlines():
            m = phyDispRE.search(dumped_line, 0)
            if m:
                displayInfo = {}
                for prop in ['width', 'height']:
                    displayInfo[prop] = int(m.group(prop))
        ...
        for dumped_line in self.shell('dumpsys window').splitlines():
            m = phyDispRE.search(dumped_line, 0)
            if not m:
                m = dispWHRE.search(dumped_line, 0)
            if m:
                displayInfo = {}
                for prop in ['width', 'height']:
                    displayInfo[prop] = int(m.group(prop))
    ...
    def __getDisplayDensity(self, key, strip=True, invokeGetPhysicalDisplayIfNotFound=True):
        ...
        if self.__displayInfo and 'density' in self.__displayInfo:  # and self.__displayInfo['density'] != -1:
    ...
    def takeSnapshot(self, reconnect=False):
        ...
        try:
            ...
            raise EnvironmentError("You have to install PIL to use takeSnapshot()")
        ...
        (version, bpp, size, width, height, roffset, rlen, boffset, blen, goffset, glen, aoffset, alen) = struct.unpack(
            '<' + 'L' * 13, received)
        if DEBUG:
        ...
        if DEBUG:
            print >> sys.stderr, "    takeSnapshot:", (
                version, bpp, size, width, height, roffset, rlen, boffset, blen, goffset, blen, aoffset, alen, argMode)
        if argMode == 'BGRA':
            argMode = 'RGBA'
        if bpp == 16:
            mode = 'RGB'
    ...
    def touchDip(self, x, y, orientation=-1, eventType=DOWN_AND_UP):
        if DEBUG_TOUCH:
            print >> sys.stderr, "touchDip(x=", x, ", y=", y, ", orientation=", orientation, ", eventType=", \
                eventType, ")"
        self.__checkTransport()
        if orientation == -1:
            orientation = self.display['orientation']
    ...
    def longTouch(self, x, y, duration=3000, orientation=-1):
        """
        Long touches at (x, y)
