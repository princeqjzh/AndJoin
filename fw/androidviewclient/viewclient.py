...
from fw.androidviewclient.common import _nd, _nh, _ns, obtainPxPy, obtainVxVy, \
    obtainVwVh, obtainAdbPath
from fw.androidviewclient.window import Window
from fw.androidviewclient import adbclient
...
class View:
    ...
    def factory(arg1, arg2, version=-1, forceviewserveruse=False, windowId=None):
        ...
        if isinstance(arg1, types.ClassType):
            cls = arg1
            attrs = None
        else:
            cls = None
    ...
    def __init__(self, _map, device, version=-1, forceviewserveruse=True, windowId=None):
        """
        Constructor
        ...
        if DEBUG_VIEW:
            print >> sys.stderr, "View.__init__(%s, %s, %s, %s)" % (
                "map" if _map is not None else None, device, version, forceviewserveruse)
            if _map:
    ...
    def setTag(self, tag):
        """
        Sets the tag.
        2015-08-31: Shawn Wang: added to set tag to the child view's index among the children of the view's parent
        to support retrieving the children path used in method __getChildView() of C{iDevice}
        """

        try:
            self.map[self.tagProperty] = tag
        except:
            pass
    ...
    def getXY(self, debug=False):
        ...
        parent = self.parent
        if DEBUG_COORDS: print >> sys.stderr, "   getXY: x=%s y=%s parent=%s" % (
            x, y, parent.getUniqueId() if parent else "None")
        hx = 0
        ...
        while parent is not None:
            if DEBUG_COORDS: print >> sys.stderr, "      getXY: parent: %s %s <<<<" % (
                parent.getClass(), parent.getId())
            if SKIP_CERTAIN_CLASSES_IN_GET_XY_ENABLED:
                if parent.getClass() in ['com.android.internal.widget.ActionBarView',
                    ...
                    if DEBUG_COORDS: print >> sys.stderr, "   getXY: skipping %s %s (%d,%d)" % (
                        parent.getClass(), parent.getId(), parent.__getX(), parent.__getY())
                    parent = parent.parent
                    continue
            if DEBUG_COORDS: print >> sys.stderr, "   getXY: parent=%s x=%d hx=%d y=%d hy=%d" % (
                parent.getId(), x, hx, y, hy)
            hx += parent.__getX()
            hy += parent.__getY()
            parent = parent.parent
        ...
        try:
            # Shawn Wang: comment out the branch because the self.windownId is always "x1", not the current
            # focused window id. therefore the code will always trap into the exception and bypass the statusbar offset
            # logic.
            # if self.windowId:
            #     print self.windowId
            ...
            #         # print key
            #         print "window %s: %s" % (key, self.windows[key])
            #     fw = self.windows[self.windowId]
            # else:
            fw = self.windows[self.currentFocus]
            if DEBUG_STATUSBAR:
                print >> sys.stderr, "    getXY: focused window=", fw
                print >> sys.stderr, "    getXY: deciding whether to consider statusbar offset because " \
                                     "current focused windows is at", (fw.wvx, fw.wvy), "parent", (fw.px, fw.py)
        except KeyError:
            print 'KeyError.'
            fw = None
        ...
        if fw:
            ...
            if fw.py == fw.wvy:
                if DEBUG_STATUSBAR:
                    print >> sys.stderr, "        getXY: but wait, fw.py == fw.wvy so we are adjusting by ", (
                        fw.px, fw.py)
                pwx = fw.px
                pwy = fw.py
            else:
                if DEBUG_STATUSBAR: print >> sys.stderr, "    getXY: fw.py=%d <= fw.wvy=%d, no adjustment" % (
                    fw.py, fw.wvy)
        else:
            print >> sys.stderr, "ERROR:    getXY: focused window not found."
        if DEBUG_COORDS or DEBUG_STATUSBAR or debug:
            print >> sys.stderr, "   getXY: returning (%d, %d) ***" % (
    ...
    def __obtainStatusBarDimensionsIfVisible(self):
        ...
        for winId in self.windows:
            ...
            if DEBUG_COORDS:
                print >> sys.stderr, "      __obtainStatusBarDimensionsIfVisible: w=", w, \
                    "   w.activity=", w.activity, "%%%"
            if w.activity == 'StatusBar':
                if w.wvy == 0 and w.visibility == 0:
                    if DEBUG_COORDS: print >> sys.stderr, "      __obtainStatusBarDimensionsIfVisible: statusBar=", (
                        w.wvw, w.wvh)
                    sbw = w.wvw
                    sbh = w.wvh
                break
    ...
    def add(self, child):
        ...
        # 2015-08-31: Shawn Wang: added to support the __getChildView() of C{iDevice}
        child.setTag(len(self.children)-1)
...
class UiDevice:
    ...
    def openQuickSettingsSettings(self):
        ...
        STATUS_BAR_SETTINGS_SETTINGS_BUTTON = [
            ...
            u"ቅንብሮች", u"การตั้งค่า", u"Seaded", u"Iestatījumi", u"Innstillinger", u"Подешавања", u"الإعدادات",
            u"සැකසීම්",
...
class ViewClient:
    ...
    def __init__(self, device, serialno, adb=None, autodump=True, forceviewserveruse=False, localport=VIEW_SERVER_PORT,
        ...
        self.useUiAutomator = (self.build[VERSION_SDK_PROPERTY] >= 16) and not forceviewserveruse  # jellybean 4.1 & 4.2
        if DEBUG:
            print >> sys.stderr, "    ViewClient.__init__: useUiAutomator=", self.useUiAutomator, "sdk=", self.build[
                VERSION_SDK_PROPERTY], "forceviewserveruse=", forceviewserveruse
    ...
    def setAlarm(timeout):
        ...
        if osName.startswith('Windows'):  # alarm is not implemented in Windows
            return
            # Shawn Wang: disabled due to unpredictable time required to dump complex views
    ...
    def connectToDeviceOrExit(timeout=60, verbose=False, ignoresecuredevice=False, ignoreversioncheck=False,
        ...
        if serialno is None:
            ...
            serialno = args[1] if len(args) > 1 else \
                os.environ['ANDROID_SERIAL'] if 'ANDROID_SERIAL' in os.environ.keys() else '.*'
        if IP_RE.match(serialno):
    ...
    def traverseShowClassIdAndText(view, extraInfo=None, noExtraInfo=None, extraAction=None):
        ...
        try:
            ...
            return u'Exception in view=%s: %s:%s\n%s' % (
                view.__smallStr__(), sys.exc_info()[0].__name__, e, traceback.format_exc())
    ...
    @staticmethod
    def __traverseAndFindViewsWithSameId(root, view_id):
        if not root:
            return False

        if root.getId() == view_id:
            if DEBUG: print "view {} has the same id {}".format(root.getId(), view_id)
            return True
        # if stream and s:
        ...
        for ch in root.children:
            if ViewClient.__traverseAndFindViewsWithSameId(ch, view_id):
                return True

    def findViewsWithSameId(self, child_view, node_path):
        """
        find views having the same id with the given view.
        if the id is unique in the tree, return b{True}. if not, find the path to identify the child view:
        <parent view id>(path index separated by comma)

        2015-08-31: added by Shawn Wang
        @param child_view: the target view
        @param node_path: a list to save the parent views on the node path. the child view should be the only
        element when assigned.
        @return:
        """
        find = False
        # if len(node_path) == 0:
        #     node_path.append(child_view)
        parent = child_view.parent

        if parent:
            if DEBUG: print 'appending parent {} to parents'.format(parent.getId())
            node_path.append(parent)

            for ch in parent.children:
                if ch.getTag() != child_view.getTag():
                    if DEBUG: print 'searching branch {}:{}...'.format(ch.getTag(), ch.getId())
                    if ViewClient.__traverseAndFindViewsWithSameId(ch, node_path[0].getId()):
                        find = True
                        break

        if not find:
            if parent:
                self.findViewsWithSameId(parent, node_path)
            else:
                print "No duplicated names, you can use the view id directly."
                return find
        else:
            # print node_path
            ch_path = node_path[::-1]  # this do a slice with a direction of -1 and return the slice;
            # or do node_path.reverse and use the altered node_path afterwards as reverse() does not return the list
            # if DEBUG: print ch_path
            print 'find views with the same name. please use below string in your testcase:'
            parent = ch_path.pop(0)

            while parent.getId() == 'NO_ID':
                node_path.insert(0, parent)
                parent = parent.parent
                if parent is None:
                    raise ValueError("Could not find a parent with a valid ID, please contact dev team for revision.")

            s = ','.join(str(p.getTag()) for p in ch_path)
            # path += parent.getTag()

            print "check {}({}) {}".format(parent.getId(), s, ch_path[-1].getText())
            return True
    ...
    def setViews(self, received, windowId=None):
        ...
        self.views = []
        ''' The list of Views represented as C{str} obtained after splitting it into lines after being received from
        the server. Done by L{self.setViews()}. '''
    ...
    def __parseTree(self, receivedLines, windowId=None):
        """
        Parses the View tree contained in L{receivedLines}. The tree is created and the root node
        assigned to L{self.root}.
        ...
        for v in receivedLines:
            ...
            if not self.root:
                ...
                if DEBUG:
                    self.root.raw = v
                    print 'root created. tree level=0'
                treeLevel = 0
                newLevel = 0
                lastView = self.root
                parent = self.root
    ...
    def dump(self, window=-1, sleep=1):
        ...
        if self.useUiAutomator:
            ...
            received = unicode(self.device.shell('uiautomator dump %s /dev/tty >/dev/null' % (
                '--compressed' if self.getSdkVersion() >= 18 and self.compressedDump else '')), encoding='utf-8',
                               errors='replace')
            if not received:
            ...
            if onlyKilledRE.search(received):
                ...
                if self.device.shell('ps | grep "%s"' % MONKEY):
                    extraInfo = "\nIt is know that '%s' conflicts with 'uiautomator'. Please kill it and try again." % \
                                MONKEY
                raise RuntimeError(
                    '''ERROR: UiAutomator output contains no valid information. UiAutomator was killed,
                    no reason given.''' + extraInfo)
            ...
            received = received.replace(
                'WARNING: linker: libdvm.so has text relocations. This is wasting memory and is a security risk. '
                'Please fix.\r\n', '')
...
You should force ViewServer back-end.''')
            ...
            # ViewClient.setAlarm(120)  # 120 seconds would not be enough for dumping complex views
            # try:  # Shawn Wang: added to catch the alarm exception so that main thread won't quit 2015-08-11
            # with time_limit(300):
            while True:
                if DEBUG_RECEIVED:
                ...
                if DEBUG_RECEIVED:
                    print >> sys.stderr, "    read 1024 bytes: ...%s" % received[-24:]
                if doneRE.search(received[-7:]):
                    break
            # except TimeoutException, msg:
            #     print "Timed out! Dump failed."
            #     self.views = None
            #     return
            s.close()
            # ViewClient.setAlarm(0)
            ...
            if DEBUG:
                self.received = received
                # show the time costed by dump
                print >> sys.stderr, "dump costed: %s" % time.strftime('%M:%S', time.localtime(end_time - start_time))
            if DEBUG_RECEIVED:
                print >> sys.stderr, "received %d chars" % len(received)
                print >> sys.stderr
            ...
            if received:
                for c in received:
                    if ord(c) > 127:
                        ...
                        break
            if DEBUG_WINDOWS:
                print "DEBUG: set views for window id: %s" % hex(window)[2:]
    ...
    def findViewById(self, viewId, root="ROOT", viewFilter=None):
        ...
        if isinstance(root, types.StringType) and root == "ROOT":
    ...
    def __findViewsWithAttributeInTree(self, attr, val, root):
        ...
        if DEBUG:
            ...
            print >> sys.stderr, "__findViewWithAttributeInTree: checking if root=%s has attr=%s == %s" % \
                                 (root.__smallStr__(), attr, val)
    ...
    def __findViewWithAttributeInTree(self, attr, val, root):
        if DEBUG:
            ...
            if isinstance(root, types.StringType):
    ...
    def __findViewWithAttributeInTreeThatMatches(self, attr, regex, root, rlist=[]):
        ...
        if DEBUG:
            print >> sys.stderr, "__findViewWithAttributeInTreeThatMatches: " \
                                 "checking if root=%s attr=%s matches %s" % (root.__smallStr__(), attr, regex)
    ...
    def findViewWithAttribute(self, attr, val, root="ROOT"):
        ...
        if DEBUG:
            try:
                print >> sys.stderr, u'findViewWithAttribute({0}, {1}, {2})'.format(attr, unicode(val, encoding='utf-8',
                                                                                                  errors='replace'),
                                                                                    root)
            except:
                pass
    ...
    def findViewWithText(self, text, root="ROOT"):
        ...
        if isinstance(text, RegexType):
            ...
            #    return l[0]
            # else:
            #    print >>sys.stderr, "WARNING: findViewWithAttributeThatMatches invoked by findViewWithText returns %d
            # items." % ll
            #    return l
        else:
    ...
    def writeImageToFile(self, filename, _format="PNG", deviceart=None, dropshadow=True, screenglare=True):
        ...
        if DEBUG:
            print >> sys.stderr, "writeImageToFile: saving image to '%s' in %s format (reconnect=%s)" % (
                filename, _format, self.device.reconnect)
...
''')
                ...
                warnings.warn("ViewClient.writeImageToFile: Cannot add device art because STUDIO_DIR environment "
